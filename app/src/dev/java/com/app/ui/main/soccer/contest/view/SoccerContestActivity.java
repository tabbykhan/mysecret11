package com.app.ui.main.soccer.contest.view;
...
import com.app.ui.main.soccer.contest.fragment.SoccerCashFragment;
import com.app.ui.main.soccer.contest.fragment.SoccerPracticeFragment;
import com.app.ui.main.soccer.contest.fragment.SoccerPrivateFragment;
 ...
 */
public class SoccerContestActivity extends AppBaseActivity implements MatchTimerListener {
    ...
    public int getLayoutResourceId() {
        return R.layout.activity_soccer_constest;
    }
    ...
    public void initializeComponent() {
        ...
        if (getMatchModel() != null) {
            ...
            if (toolbar instanceof AppBaseFragment) {
                ...
                toolbarFragment.setToolbarView(SoccerContestActivity.this);
            }
    ...
    private void setupViewPager() {
        ...
        adapter1.addFragment(new SoccerCashFragment(), "CASH");
        adapter1.addFragment(new SoccerPracticeFragment(), "PRACTICE");
        adapter1.addFragment(new SoccerPrivateFragment(), "PRIVATE");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.ll_my_contest: {
                Bundle bundle = new Bundle();
                bundle.putString(DATA, getMatchModel().getMatch_id());
                bundle.putString(DATA1, SoccerContestActivity.this.getClass().getSimpleName());
    ...
    private void goToMyTeamsActivity(Bundle bundle) {
        Intent intent = new Intent(this, SoccerMyTeamsActivity.class);
        if (bundle != null) {
            intent.putExtras(bundle);
        }
        startActivity(intent);
    ...
    private void goToMyContestActivity(Bundle bundle) {
        Intent intent = new Intent(this, SoccerMyContestActivity.class);
        if (bundle != null) {
            intent.putExtras(bundle);
        }
        startActivity(intent);
    ...
    public void getMatchContest(JSONObject jsonObject) {
        if (getMatchModel() != null) {
            try {
                ...
                ((SoccerCashFragment) item).showRefreshing();


                item = adapter1.getItem(1);
                ((SoccerPracticeFragment) item).showRefreshing();
            } catch (Exception e) {

            }

            getWebRequestHelper().getSoccerMatchContest(getMatchModel().getId(), getMatchModel().getMatch_id(), jsonObject, this);
        }
    }
            ...
            case ID_SOCCER_MATCH_CONTEST:
                ...
                if (responsePojo != null) {
                    if (responsePojo.getData().size() > 0 && adapter1 != null) {
                        SoccerCashFragment item = (SoccerCashFragment) adapter1.getItem(0);
                        if (item != null) {
    ...
    private void handleMatchContestResponse(WebRequest webRequest) {
        ...
        if (!responsePojo.isError()) {
            ...
            if (data != null && data.size() > 0) {
                ...
                ((SoccerCashFragment) item).setupDetsil(detailBean);
                ((SoccerCashFragment) item).setupData(contestCategoryModels);
            }else{
            ...
            item = adapter1.getItem(1);
            ((SoccerPracticeFragment) item).setupDetsil(detailBean);
            ((SoccerPracticeFragment) item).setupData(practiceCategoryModels);
