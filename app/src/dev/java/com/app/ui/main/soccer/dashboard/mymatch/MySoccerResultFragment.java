...
import com.app.ui.main.soccer.dashboard.mymatch.adapter.MySoccerResultAdapter;
 ...
 */
public class MySoccerResultFragment extends AppBaseFragment implements MatchTimerListener {

    private MySoccerResultAdapter adapter;
    ...
    public int getLayoutResourceId() {
        return R.layout.fargment_my_result;
    }
    ...
    private void initializeRecyclerView() {
        adapter = new MySoccerResultAdapter(getActivity(), MyApplication.getInstance().getMyPastMatches()) {
            @Override
            ...
            public void onItemClicked(RecyclerView recyclerView, int position, View v) {
                try {
                    ...
                    if (matchModel != null) {
                        ...
                        bundle.putString(DATA1, MySoccerResultFragment.this.getClass().getSimpleName());
    ...
    public void onPageSelected() {
        ...
        getCustomerPastMatches();
    }

    private void getCustomerPastMatches() {
        swipe_layout.setRefreshing(true);
        updateViewVisibitity(ll_no_item, View.GONE);
        updateViewVisibitity(tv_no_item, View.GONE);
        getWebRequestHelper().getCustomerResultLiveMatches(this);
    }
    ...
    public void onWebRequestResponse(WebRequest webRequest) {
        ...
        switch (webRequest.getWebRequestId()) {
            case ID_CUSTOMER_SOCCER_RESULT_MATCHES:
                handleCustomerPastMatchesResponse(webRequest);
                break;
        }
    ...
    private void handleCustomerPastMatchesResponse(WebRequest webRequest) {
        ...
        if (!responsePojo.isError()) {
            synchronized (MyApplication.getInstance().getLock()) {
                ...
                MyApplication.getInstance().getMyPastMatches().clear();
                if (data != null && data.size() > 0) {
                    MyApplication.getInstance().getMyPastMatches().addAll(data);
                }
                if (isFinishing()) return;
    ...
    private void updateNoDataView() {
        if (MyApplication.getInstance().getMyPastMatches().size() > 0) {
            updateViewVisibitity(ll_no_item, View.GONE);
            updateViewVisibitity(tv_no_item, View.GONE);
        } else {
            tv_no_item.setText("No past result available");
